...
// [SECTION] ImGui end-user API functions
...
struct ImGuiContext;                // ImGui context (opaque structure, unless including imgui_internal.h)
...
namespace ImGui
{
    ...
    IMGUI_API void          EndFrame();                                 // ends the ImGui frame. automatically called by Render(). If you don't need to render data (skipping rendering) you may call EndFrame() without Render()... but you'll have wasted CPU already! If you don't need to render, better to not create any windows and not call NewFrame() at all!
    IMGUI_API void          Render();                                   // ends the ImGui frame, finalize the draw data. You can then get call GetDrawData().
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after Render() and until the next call to NewFrame(). this is what you have to render.

    // Demo, Debug, Information
    IMGUI_API void          ShowDemoWindow(bool* p_open = NULL);        // create Demo window. demonstrate most ImGui features. call this to learn about the library! try to make it always available in your application!
    IMGUI_API void          ShowMetricsWindow(bool* p_open = NULL);     // create Metrics/Debugger window. display ImGui internals: windows, draw commands, various internal state, etc.
    ...
    IMGUI_API void          ShowAboutWindow(bool* p_open = NULL);       // create About window. display ImGui version, credits and build/system information.
    ...
    // - Not that MenuItem() keyboardshortcuts are displayed as a convenience but _not processed_ by ImGui at the moment.
    ...
    // Viewports
    // - Currently represents the Platform Window created by the application which is hosting our ImGui windows.
...
// Note: if you are trying to check whether your mouse should be dispatched to ImGui or to your app, you should use 'io.WantCaptureMouse' instead! Please read the FAQ!
...
// Standard Drag and Drop payload types. You can define you own payload types using short strings. Types starting with '_' are defined by ImGui.
...
enum ImGuiConfigFlags_
{
    ...
    // User storage (to allow your backend/engine to communicate to code that may be shared between multiple projects. Those flags are NOT used by core ImGui)
...
struct ImGuiIO
{
    ...
    bool        WantCaptureMouse;                   // Set when ImGui will use mouse inputs, in this case do not dispatch them to your main game/application (either way, always pass on mouse inputs to imgui). (e.g. unclicked mouse is hovering over an imgui window, widget is active, mouse was clicked over an imgui window, etc.).
    bool        WantCaptureKeyboard;                // Set when ImGui will use keyboard inputs, in this case do not dispatch them to your main game/application (either way, always pass keyboard inputs to imgui). (e.g. InputText active, or an imgui window is focused and navigation is enabled, etc.).
    bool        WantTextInput;                      // Mobile/console: when set, you may display an on-screen keyboard. This is set by ImGui when it wants textual keyboard input to happen (e.g. when a InputText widget is active).
    ...
    bool        MouseDown[5];                       // Mouse buttons: 0=left, 1=right, 2=middle + extras (ImGuiMouseButton_COUNT == 5). ImGui mostly uses left and right buttons. Others buttons allows us to track if the mouse is being used by your application + available to user as a convenience via IsMouse** API.
    ...
    float       PenPressure;                        // Touch/Pen pressure (0.0f to 1.0f, should be >0.0f only when MouseDown[0] == true). Helper storage currently unused by ImGui.
...
// (ImGui already clip items based on their bounds but: it needs to first layout the item to do so, and generally
...
struct ImDrawList
{
    ...
    //   In older versions (until ImGui 1.77) the AddCircle functions defaulted to num_segments == 12.
...
// All draw data to render a ImGui frame
...
//  - One or more fonts.
//  - Custom graphics data needed to render the shapes needed by ImGui.
...
struct ImGuiViewport
{
    ...
    ImVec2              Pos;                    // Main Area: Position of the viewport (ImGui coordinates are the same as OS desktop/native coordinates)
